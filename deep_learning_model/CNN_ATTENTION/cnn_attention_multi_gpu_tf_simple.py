...
def bulid_model(sentences, num_words, DIM, num_classes, labels, num_filters):
    ...
    with tf.variable_scope("attetion", reuse=tf.AUTO_REUSE):
        ...
        input_projection = tf.contrib.layers.fully_connected(bn_conv1, num_filters*2, activation_fn=tf.tanh)
        # 输出 [batch_size, units]
        vector_attn = tf.reduce_sum(tf.multiply(input_projection, attention_context_vector), axis=2, keep_dims=True)
        attention_weights = tf.nn.softmax(vector_attn, dim=1)

        weighted_projection = tf.multiply(bn_conv1, attention_weights)
        attentnion_outputs = tf.reduce_sum(weighted_projection, axis=1)

        bn_attention = tf.layers.batch_normalization(attentnion_outputs,
                                           momentum=0.99,
                                           training=True,
                                           )

        fully_connect = tf.layers.dense(bn_attention,
                                        num_filters,
                                        activation=tf.nn.relu)
