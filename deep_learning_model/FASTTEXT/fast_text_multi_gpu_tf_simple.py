...
flags.DEFINE_integer('num_filters', 1024, 'the number of filters')
...
flags.DEFINE_integer('DIM', 512, 'the dim of embedding vector')
...
def bulid_model(sentences, num_words, DIM, num_classes, labels, num_filters):
    ...
    with tf.variable_scope('global_layer', reuse=tf.AUTO_REUSE):
        # cnn + global_max_pooling
        global_max = tf.reduce_max(sentences, 1)
        global_mean = tf.reduce_mean(sentences,1)
        pool_concat = tf.concat([global_max, global_mean], axis=1)
