...
def bulid_model(sentences, num_words, DIM, num_classes, labels, num_filters):
    ...
    with tf.variable_scope('conv', reuse=tf.AUTO_REUSE):
        ...
        global_pool1 = tf.reduce_max(conv1, 1)

        conv2 = tf.layers.conv1d(sentences, num_filters, 15, 1, 'same',
                                 kernel_initializer=tf.contrib.layers.xavier_initializer(),
                                 activation=tf.nn.relu)
        global_pool2 = tf.reduce_max(conv2, 1)

        conv3 = tf.layers.conv1d(sentences, num_filters, 17, 1, 'same',
                                 kernel_initializer=tf.contrib.layers.xavier_initializer(),
                                 activation=tf.nn.relu)
        global_pool3 = tf.reduce_max(conv3, 1)
        pool_concat = tf.concat([global_pool1, global_pool2, global_pool3], axis=1)

        bn = tf.layers.batch_normalization(pool_concat,
                                           momentum=0.90,
                                           training=True,
                                           )
