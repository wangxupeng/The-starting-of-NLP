...
flags.DEFINE_integer('num_filters', 256, 'the number of filters')
...
def bulid_model(sentences, num_words, DIM, num_classes, labels, num_filters):
    ...
    with tf.variable_scope('block', reuse=tf.AUTO_REUSE):
        # cnn + global_max_pooling
        conv1 = tf.layers.conv1d(sentences, num_filters, 16, 1, 'same',
                                 kernel_initializer=tf.contrib.layers.xavier_initializer(),
                                 activation=tf.nn.relu)


        x_conv1 = tf.add(sentences, conv1)

        block_activation_1 = tf.nn.relu(x_conv1, name="block_activation")

        global_max_pool_1 = tf.reduce_max(block_activation_1, axis=1)
        global_avg_pool_1 = tf.reduce_mean(block_activation_1, axis=1)
        global_sum_pool_1 = tf.reduce_sum(block_activation_1, axis=1)


        all_pool = tf.concat([global_max_pool_1, global_avg_pool_1, global_sum_pool_1,], axis=1)

        bn = tf.layers.batch_normalization(all_pool,
                                           momentum=0.90,
                                           training=True,
                                           )
