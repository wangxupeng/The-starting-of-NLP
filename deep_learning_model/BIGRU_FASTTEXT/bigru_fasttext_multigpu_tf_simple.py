...
def bulid_model(sentences, num_words, DIM, num_classes, labels, num_filters):
    ...
    with tf.variable_scope("fast_text", reuse=tf.AUTO_REUSE):
        global_max_pool = tf.reduce_max(bigru_outputs, axis=1)
        global_mean_pool = tf.reduce_mean(bigru_outputs, axis=1)
        global_sum_pool = tf.reduce_sum(bigru_outputs, axis=1)

        all_pool = tf.concat([global_max_pool, global_mean_pool, global_sum_pool], axis=1)
